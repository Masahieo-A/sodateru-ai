    ...
    <div className="rounded-2xl border border-amber-200 overflow-hidden">
      ...
      >
        ...
        <span className="text-amber-500 text-xs font-medium">
          {loading ? "考え中..." : prepaidDepleted ? "利用不可" : hint ? <><AppIcon name="refresh" /> もう一度</> : <><AppIcon name="next" /> 見る</>}
        </span>
      </button>

      {error && !loading && (
        <div className="bg-white px-4 py-3 space-y-2">
          <p className="text-sm text-red-600"><AppIcon name="warning" /> {error}</p>
      ...
      {hint && !loading && (
        <div className="bg-white px-4 py-4">
          <div className="flex items-start gap-2">
            <AppIcon name="magic" size={28} className="text-amber-600" />
